...
static NSString *kBaseUrl = ServerIP;  //ServerIP是宏定义的请求地址
...
+ (void)getWithPath:(NSString *)path params:(NSDictionary *)params success:(HttpSuccessBlock)success failure:(HttpFailureBlock)failure {
    ...
    [[AFHttpClient sharedClient] GET:url parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (responseObject) {
            ...
            if (code == 200) { // 成功回调
                NSDictionary *dic = [HttpTool changeType:responseObject];
                success(dic[@"mapResult"]);
            } else {
                NSString *message = responseObject[@"resultMsg"];
                [MBProgressHUD ShowErrorHUD:[NSString SafeString:message]];
            }
        } else {
            [MBProgressHUD ShowErrorHUD:@"网络请求失败"];
        }
